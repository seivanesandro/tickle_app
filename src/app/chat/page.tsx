...
export default function ChatPage() {
  ...
  // Load initial data (User and Chat List)
  useEffect(() => {
  ...
  async function handleSendMessage(content: string, imageBase64: string | null) {
    ...
    // Se nÃ£o houver conversa ativa, criar uma na hora
    if (!currentChatId) {
      ...
      currentChatId = id;
      // Atualizar lista local de chats simulando a criaÃ§Ã£o
    ...
    // UI Otimista: mostra a mensagem do utilizador imediatamente
    const tempId = Date.now().toString();
    addMessage({ id: tempId, chat_id: currentChatId as string, role: "user", content: content || "[Image sent]", created_at: new Date().toISOString() });

    try {
      // FAZER A CHAMADA REAL Ã€ API
      ...
      if (!res.ok) {
        ...
        // Adiciona a resposta real da BD e da IA Ã  store
